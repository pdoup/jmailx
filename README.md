# jmailx - CLI e-mail utility in Java
...
- Saving attachments found in e-mails can be performed by appending the `-d` option. Using this option creates a unique folder (in the same directory the program was launched) for each e-mail retrieved and stores any attachments inside. It's very common for e-mails to follow the `text/html` mimetype; if that's the case, the content of the e-mail is treated as an HTML attachment which is also downloaded when the option is specified.
...
#### Filtering engine
The user can impose specific filters on messages that are going to be displayed using the `-f` or `--filter` option. This is achieved through a custom search mechanism that filters certain terms, for instance, fetch all e-mails from folder that were sent after a certain date and also contain 'Meeting' as part of the subject. This whole process is translated to a simple expression-based filtering schema designed to combine multiple search terms, presented in the form of term-value pairs. In a nutshell, the following terms can be used as search terms:
...
_Note_: Negating an option is possible by prepending the `!` symbol before the value, for instance, 
to fetch all messages **not** containing the term 'Work' in the subject part, issue the following statement: `subject:!Work`

**Search terms can also be combined** using logical operators. To ensure 2 search terms coexist the user can join the options with a `+` symbol. Furthermore, to make sure either term is present the user can join the 2 terms with the `|` symbol. The evaluation order is from left to right. For testing reasons and to enhance clarity, a natural language translation mechanism was also developed that converts search strings to natural language clearly and concisely. For example, consider the following complete search expression `"subject:hello+from:test@gmail.com|size_le:4kb"`. This is equivalent to `((subject contains "hello" and sender is "test@gmail.com") or size less than "4096" bytes)"`. For more examples and test cases visit the [test suite](./src/test/java/iti/mail) folder.  
